...
# Configuration Paths#!/usr/bin/env bash
# ==============================================================================
# Homelab AI Deployer - Controller AMD (manager-amd.sh)
...
install_vulkan_dependencies() {
    ...
    # Pacchetti testati e compatibili con Debian 13 (Trixie)
    apt-get install -y \
        build-essential \
        cmake \
        git \
...
install_services() {
    ...
    case $amd_choice in
        ...
        2)
            ...
            # Pulizia cache CMake per evitare conflitti con vecchi percorsi
            ...
            echo -e "  ${C_YELLOW}[*] Compilazione in corso con $(nproc) thread...${RESET}"
...
show_header() {
    clear
    echo -e "${C_PURPLE}┌──────────────────────────────────────────────────────────────────────────────┐${RESET}"
    echo -e "${C_PURPLE}│${RESET}  ${BOLD}${C_WHITE}H O M E L A B   A I   D E P L O Y E R  --  C O N T R O L L E R   A M D${RESET}     ${C_PURPLE}│${RESET}"
    echo -e "${C_PURPLE}│${RESET}  ${C_GRAY}AMD Hardware Management, Vulkan/ROCm Acceleration & llama.cpp Builder${RESET}   ${C_PURPLE}│${RESET}"
    echo -e "${C_PURPLE}└──────────────────────────────────────────────────────────────────────────────┘${RESET}"
    echo ""
}
...
if [ "$EUID" -ne 0 ]; then
    show_header
    echo -e "  ${C_RED}𐄂 PERMESSI INSUFFICIENTI${RESET}"
    echo -e "  ${C_GRAY}Esegui lo script come root (sudo ./manager-amd.sh)${RESET}\n"
    exit 1
fi
