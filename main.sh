...
update_components() {
    ...
    if [ -d ".git" ]; then
        ...
        # FIX PERCORSI: Rendiamo eseguibili tutti gli script nella cartella script/ e il main
        if [ -d "script" ]; then
            chmod +x script/*.sh
        fi
        [ -f "main.sh" ] && chmod +x main.sh
        
        echo -e "${GREEN}[OK] Repository aggiornata.${NC}"
        echo -e "${YELLOW}---> Riavvio dello script aggiornato...${NC}"
        sleep 2
        
        # Riavvio dello script corretto dal nuovo percorso
